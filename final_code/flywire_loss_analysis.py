...
for i in range(n_mean_bins):
    ...
    var_bin_idx = pd.cut(df_nonneg.loc[mean_mask, 'std'], bins=var_q, labels=False, include_lowest=True)
...
for i in range(n_mean_bins):
    ...
    grouped_var = df_nonneg[mean_mask].groupby('var_bin')['std'].median()
...
plt.subplots_adjust(**fig_margins)
plt.savefig('../../paper_figures/simulated_loss/loss_vs_robustness_binned.pdf', dpi=600)
