...
SKIP_EXISTING_SIMULATIONS = False
...
def run_flywire_loss_simulation():
    ...
    output_file = data_dir + 'loss_data.parquet'
...
def run_flywire_periphery_scoring():
    ...
    output_file = data_dir + 'periphery_data.parquet'
...
def run_parametric_simulation(distribution, mean_vals, var_vals, n_inputs=100, 
...
def run_parametric_simulations():
    ...
    lomax_var_vals = 10**np.arange(1, 5)  # Lomax requires var > mean^2
    ...
    run_parametric_simulation('lomax', mean_vals, lomax_var_vals, n_inputs=n_inputs,
                               n_draws=n_draws, n_perturb=n_perturb, rng=rng)
