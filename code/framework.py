...
def plot_z_plane_stripe(
    param_sets,
    colors,
    n_inputs,
    fname="z_plane_stripe",
    stripe_scale=0.4,
    lim_factor=1.5,
    n_grid=500,
    fuzzy=False,
    ...
    """
    Plot the (z, z_tilde) plane with a symmetrized viridis gradient stripe
    along y=x: purple at the centre of the stripe, yellow at the edges.
    ...
    param_sets : list of tuples
        Each tuple is (distribution, mean) or (distribution, mean, var).
    colors : list of tuple
        RGB colors (unused for the stripe itself but kept for API consistency).
    n_inputs : int
        Number of inputs used to compute sigma_z (sets the axis scale).
    fname : str
        Filename suffix for saving.
    stripe_scale : float
        Half-width of the stripe as a fraction of the axis limit.
    lim_factor : float
        Plot limits as a multiple of the maximum sigma_z.
    n_grid : int
        Resolution of the gradient image.
    fuzzy : bool
        If True, alpha follows a Gaussian falloff (sigma = stripe half-width)
        away from y=x instead of a hard cutoff.
    """
    fig, ax = plt.subplots(figsize=(width_md, height_md))

    # Compute sigma_z for each param set to set axis limits
    sigma_zs = []
    for param_set in param_sets:
        distribution, mean, var = parse_param_set(param_set)
        sigma_z = np.sqrt(n_inputs * ((var or 0.0) + mean**2))
    ...
    lim = lim_factor * max(sigma_zs)
    unnorm_stripe_scale = stripe_scale * lim

    # Shade Q2 (x<0, y>0) and Q4 (x>0, y<0) red
    ax.fill_betweenx(np.linspace(0, lim, 2), -lim, 0, color=con_colors[1], alpha=0.3)
    ax.fill_betweenx(np.linspace(-lim, 0, 2), 0, lim, color=con_colors[1], alpha=0.3)

    # Build gradient stripe via imshow
    xy = np.linspace(-lim, lim, n_grid)
    X, Y = np.meshgrid(xy, xy)

    # Horizontal gradient: 0 at x=0 (purple), 1 at x=±lim (yellow)
    dist = np.abs(X) / lim

    cmap = plt.get_cmap("viridis")
    rgba = cmap(dist)                          # (n_grid, n_grid, 4)

    # Alpha: hard cutoff or Gaussian falloff perpendicular to y=x
    norm_perp = (Y - X) / unnorm_stripe_scale
    if fuzzy:
        rgba[..., 3] = np.exp(-0.5 * norm_perp**2)
    else:
        rgba[..., 3] = np.where(np.abs(norm_perp) <= 1.0, 0.7, 0.0)
    ...
    # Coordinate axes
    ax.axhline(0, color="k", alpha=0.5, lw=1, zorder=2)
    ax.axvline(0, color="k", alpha=0.5, lw=1, zorder=2)
...
x_range_pdf = (1e-2 if pdf_log_x else 0, max(x_max_vals) if not pdf_log_x else 10*max(x_max_vals))
...
plot_z_plane_stripe(param_sets, sim_colors, n_inputs)
print("  Generated z-plane stripe figure")

plot_z_plane_stripe(param_sets, sim_colors, n_inputs, fname="z_plane_stripe_fuzzy", fuzzy=True)
print("  Generated z-plane fuzzy stripe figure")
